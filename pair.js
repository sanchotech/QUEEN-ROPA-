...
const config = {
    ...
    IMAGE_PATH: 'https://files.catbox.moe/j886mk.jpg',
    ...
    BUTTON_IMAGES: {
        ALIVE: 'https://files.catbox.moe/2mte1r.jpg',
        MENU: 'https://files.catbox.moe/j886mk.jpg',
        OWNER: 'https://files.catbox.moe/j886mk.jpg',
        SONG: 'https://files.catbox.moe/2mte1r.jpg',
        VIDEO: 'https://files.catbox.moe/2mte1r.jpg'
    }
};
...
async function SendSlide(socket, jid, newsItems) {
    ...
    for (let item of newsItems) {
        ...
        try {
            ...
            imgBuffer = await Jimp.read('https://files.catbox.moe/j886mk.jpg');
        ...
        try {
            switch (command) {   
                ...
                case 'alive': {
                    ...
                    const title = '*QUEEN ROPA*';
                    const content = `*QUEEN ROPA*\n` +                                   `ʙᴏᴛ ᴏᴡɴᴇʀ :- *SANCHO TECH*\n` +
                                `*ʙᴏᴛ ɴᴀᴍᴇ :- QUEEN ROPA \n` +
                                   `*ʙᴏᴛ ᴡᴇʙ ꜱɪᴛᴇ*\n` +
                                   `> *https*`;
...
//=======================================
  ...
  if (!number) {
    return await socket.sendMessage(sender, {
      text: '❎ *Usage:* .pair +263780325XXX \n\n*Example:* .pair +263780325289',
      quoted: msg
    });
  }
  ...
  if (!/^\+\d{10,15}$/.test(number)) {
    return await socket.sendMessage(sender, {
      text: '❎ *Invalid phone number.* Please use a valid number starting with "+" (e.g., +263780325289).',
      quoted: msg
    });
  }
  ...
  break;
                 ...
                 case 'owner': {
                    ...
                    await socket.sendMessage(sender, {
                        contacts: {
                            displayName: QUEEN ROPA",
                            contacts: [{ vcard }]
                        },
                        image: { url: config.BUTTON_IMAGES.OWNER },
                        caption: '*𝚙𝚘𝚠𝚎𝚛𝚎𝚍 𝚋𝚢 SANCHO TECH,
                        buttons: [
    ...
    await socket.sendMessage(sender, {
        image: { url: "https://files.catbox.moe/2mte1r.jpg" },
        caption: formatMessage(title, content, footer)
    });
    break;
                }
    ...
    const buttonMessage = {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          sourceUrl: "https://whatsapp.com/channel/0029VbBE7xbFCCoTEjmgV73v",
          mediaType: 1,
...
await socket.sendMessage(userJid, {
    ...
    caption: formatMessage(
        ...
        'QUEEN ROPA'
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'QUEEN ROPA'}`);
                }
            }
        });
    } catch (error) {
